...
func leaveProject(projectName string) {
	...
	for _, item := range project {
		...
		client2 := &http.Client{}
		req2, _ := http.NewRequest("PUT", auth.BaseURL+"/v1/project/leave/"+projectID, nil)
		req2.Header.Set("authorization", "Bearer "+accessToken)
		res2, _ := client2.Do(req2)
		if res2.Status != "200 OK" {
			fmt.Printf("\nerror:Unable to remove you as Team Member: %v\n\ninfo: mostly internal error with backend :(. \nPlease try again with good internet or rant/abuse maintainers \n", projectName)
			os.Exit(1)
		} else {
			fmt.Printf("success: Successfully left project: " + projectName + "\n")
		}
...
var leaveCmd = &cobra.Command{
	Use:   "leave",
	Short: "Use command to leave a project",
	Long: `Usage:
	acm leave (projectname)
	
tip: after a sucess message use acm projects (projectName)
	  to see detailed info`,
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) != 1 {
			fmt.Println(" error : command requires the name of the project you wanna leave as argument.\n put in quotes if it contains a space or more than one word")
		} else {
			leaveProject(args[0])
		}
	},
